/*********************************************************************
  OpenElevator -- Elevator Simulator
 **********************************************************************
  Platform : Teensy 4.1
  ...
  SSD1306 OLED  -- 128x64 SW-SPI (U8g2)
  Rotary enc    -- navigation + long-press for config mode
  74HC165 chain -- floor-call buttons, one register per 8 floors
  74HC595 chain -- matching button LEDs, same chain length
  Door relays   -- trigger prop/servo door controller

  Configurable (EEPROM)
  -----------------------------------------------------------------
  numFloors           2 to MAX_FLOORS
  floorLabels[]       up to LABEL_LEN chars each
...
constexpr uint8_t PIN_ENC_BTN = 22;

constexpr uint8_t PIN_SR_LOAD = 25;   // 74HC165 parallel load, active LOW
...
// =================================================================
//  COMPILE-TIME LIMITS
//  MAX_FLOORS is the only value to change when expanding the chain.
//  Add one 74HC165 and one 74HC595 per 8 additional floors.
//  NUM_SR_BYTES is derived automatically.
// =================================================================

constexpr uint8_t  MAX_FLOORS   = 64;
constexpr uint8_t  NUM_SR_BYTES = (MAX_FLOORS + 7) / 8;
constexpr uint8_t  LABEL_LEN    = 4;
constexpr uint16_t EEPROM_MAGIC = 0xE1A7;
constexpr uint16_t EEPROM_BASE  = 0;

// =================================================================
//  TYPES
//  All enums and structs are defined here, before any functions,
//  so that arduino-cli's auto-generated forward declarations have
//  these types in scope.
// =================================================================

enum DispatchMode : uint8_t {
  DISPATCH_COLLECTIVE = 0,
  DISPATCH_NEAREST    = 1,
  DISPATCH_MANUAL     = 2
};
...
struct ElevatorRuntime {
  ...
  bool          callPending[MAX_FLOORS];
  bool          hallCallUp[MAX_FLOORS];
  bool          hallCallDown[MAX_FLOORS];
};
...
//  +--------------------------------------------------------------+
//
//  Left strip: hollow square = current floor, filled disc = pending call.
//  When numFloors > 10 the strip scrolls to keep current floor visible.
// =================================================================
...
void drawHUD() {
  ...
  for (uint8_t slot = 0; slot < DOT_SLOTS && (windowBase + slot) < cfg.numFloors; slot++) {
    ...
    if (here)    u8g2.drawFrame(2, y - 2, 5, 5);
    if (pending) u8g2.drawDisc(4, y, 2, U8G2_DRAW_ALL);
  }

  uint8_t halfOpen = (uint8_t)(el.doorPosition * 58.0f);
  u8g2.drawBox(0,             56, 64 - halfOpen, 8);
  u8g2.drawBox(64 + halfOpen, 56, 64 - halfOpen, 8);
...
// =================================================================
//  BUTTON SHIFT-REGISTER SCAN  (74HC165, active LOW)
// =================================================================

namespace Buttons {
  uint8_t current[NUM_SR_BYTES] = {};
  uint8_t prev[NUM_SR_BYTES]    = {};
  uint8_t fell[NUM_SR_BYTES]    = {};
...
namespace LEDs {
  ...
  void update() {
    ...
    set(el.currentFloor, (millis() / 250) & 1);   // blink at 2 Hz
    flush();
  }
}
